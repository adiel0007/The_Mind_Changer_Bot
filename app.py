...
# 1. הגדרת עמוד Streamlit למצב רוחב מלא (Wide Mode)
st.set_page_config(
    page_title="The Mind Changer | Stock Radar",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# 2. הזרקת CSS מיוחד ל-Streamlit כדי להעלים ריפודים (Padding) ושוליים מהמסגרת המרכזית
st.markdown("""
    <style>
        /* העלמת רווחים מובנים של Streamlit */
        .block-container {
        ...
        /* ביטול מרווחי ברירת מחדל של אלמנט ה-iframe */
        iframe {
            display: block;
        }
        /* הסתרת כפתורי המניו המובנים של Streamlit (אופציונלי, למראה נקי) */
...
# 3. הגדרת קוד ה-HTML המלא שלך בתוך משתנה טקסט
html_code = """
<!DOCTYPE html>
...
<section id="features">
  <div class="section-wrap">
    ...
    <div class="features-grid">
      ...
      <div class="feat-card"><span class="feat-icon">🤖</span><div class="feat-title">אנליסט AI — Gemini</div><div class="feat-desc">ניתוח עמוק לכל מניה ומענה לשאלות פיננסית בעברית</div></div>
...
# 4. הצגת רכיב ה-HTML עם הגדרות רוחב וגובה שמתאימות לדף ארוך
# המאפיין scrolling=True מאפשר גלילה חלקה בתוך המכל של ה-HTML במידת הצורך
