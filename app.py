...
# (כרטיסיות 1 ו-2 מוגנות באמצעות סשן דינמי למניעת קריסות)
with tab1:
...
# ==================== כרטיסיית מניה בודדת ו-AI (חסינת חסימות קצב) ====================
with tab3:
            ...
            with analysis_container:
                if search_ticker:
                    with st.spinner("מבצע ניתוח מעמיק ומנטרל חסימות שרת..."):
                        # ריענון כותרות אקראי למניעת חסימת IP
                        session.headers.update(get_random_headers())
                        t = yf.Ticker(search_ticker, session=session)
                        
                        # הגנה מפני RateLimitError של Yahoo Finance
                        try:
                        ...
                        # במקרה שאין חסימה - מחשבים מדדים כרגיל
                        if not rate_limit_hit:
                            ...
                            # 🚨 מצב גיבוי (Fallback) - אם יש חסימת קצב מ-Yahoo, ה-AI משלים הכל!
                            st.warning("⚠️ שרתי Yahoo עמוסים. מפעיל מערכת גיבוי חכמה מבוססת אנליסט AI...")
                        ...
                        # שאילתת הנתונים המלאה והמשולבת ל-Gemini AI עבור סעיף 7 וחוות הדעת
                        ai_prompt = (
                        ...
                        # תצוגה מעוצבת ונקייה במבנה ה-Premium המקורי
