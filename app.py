...
# ═══════════════════════════════════════════════════════
#  הזרקת הסטייל המקורי והמדויק של האתר לתוך המערכת
# ═══════════════════════════════════════════════════════
st.markdown("""
<style>
/* עיצוב רקע כללי קשיח של האתר */
.stApp {
    background: #0a0a08 !important;
    color: #f0ede6 !important;
}

/* התאמת אזור הטאבים לעיצוב המקורי של האתר */
...
def do_scan(mode):
    ...
    # חיווי התקדמות מעל אזור הרדאר בעיצוב זהב נקי
...
# שימוש בכרטיסיות מובנות שמעוצבות בדיוק לפי ה-Tab Bar המקורית שלך
...
with tab_long:
    ...
    with radar_col1:
        ...
        # כפתור סריקה מעוצב בדיוק לפי ה-CSS המקורי שלך
...
with tab_ai:
    ...
    with radar_col5:
        ...
        ticker_val = st.text_input("ticker", key="ticker_clean", placeholder="AAPL, TSLA, NVDA...", label_visibility="collapsed")
        ...
        if st.button("נתח מניה", key="btn_a"):
            if st.session_state.ticker_clean:
                st.session_state.analysis = analyze_ticker(st.session_state.ticker_clean.upper())
                st.rerun()
    ...
    with radar_col6:
        ...
        const QA_CLEAN=[
          ...
          ['לונג','לונג = קנייה רגילה מתוך ציפייה לעלייה. קונים בזול, מוכרוים ביוקר.'],
