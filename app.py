...
/* עיצוב לכפתורי סינון עומק מחזורי */
...
/* שדות קלט למניעת מצבי טקסט לבן על רקע לבן */
...
def analyze_ticker(ticker):
    try:
        ...
        # ── 2. חוות דעת פיננסית מעודכנת, נקודתית ומקצועית ומבנה ברור ──
        ...
        macro_trend = "מעל קו המגמה הראשי" if last > ma200 else "מתחת לקו המגמה הראשי"

        formatted_opinion = f"🎯 <b>מסקנה אנליטית:</b> מניית {ticker} נמצאת כעת במבנה מחירים <b>{trend_status}</b> בטווח הקצר המיידי.<br/>" \
                            ...
                            f"🌐 <b style='color:#c9a84c;'>טווח ארוך (מאקרו):</b> המניה נסחרת <b>{macro_trend} (MA200)</b> הממוקם בשער של ${ma200:.2f}, המשמש ציר טכני אסטרטגי.<br/>" \
...
# ── 1. תיקון הרמטי: שרשור שורות ללא רווחים בתחילת שורה למניעת קריסת Markdown ──
def render_analysis(d):
    ...
    def make_row(label, val_text, badge_text="", is_pos=None):
        ...
        return f'<div style="display: flex; justify-content: space-between; align-items: center; padding: 11px 16px; border-bottom: 1px solid rgba(255,255,255,0.04); direction: rtl;">' \
    ...
    rows_html = make_row("RSI (14)", d.get("rsi_val", ""), d.get("rsi_status", ""), rsi_pos) + \
                make_row("ממוצע נע 9 ימים", d.get("ma_val", ""), d.get("ma_status", ""), ma_pos) + \
                make_row("מגמת מאקרו (MA200)", d.get("ma200", ""), "קו מגמה", None) + \
                make_row("סנטימנט אופציות", d.get("options", ""), "", None) + \
                make_row("דוחות כספיים", d.get("earnings", ""), "רבעון", None) + \
    ...
    html = f'<div class="result-card" style="border: 1px solid rgba(201,168,76,0.15); background: #11110e; border-radius: 4px; overflow: hidden; margin-top: 15px;">' \
           ...
           f'<small style="color: {\'#16a34a\' if d.get(\'up\') else \'#dc2626\'}; font-size: 0.75rem; margin-right: 6px; font-family: \'Inter\'; font-weight:600;">{d.get("chg", "")}</small></span>' \
...
<div class="panel-card" style="margin-top:15px; border-bottom-left-radius:0; border-bottom-right-radius:0; padding-bottom:10px;">
  ...
  <div class="panel-sub">שאל שאלות פיננסיות וקבל הסברים</div>
        ...
        # ── 3. תיקון מנוע חיפוש: תמיכה מלאה בשאלות משתמש טבעיות, מונחים ומניות ספציפיות ──
        if st.button("שאל", key="qa_trigger"):
            if qa_val:
                q = qa_val.strip().lower()
                match_text = None
                
                # א. בדיקה אם המשתמש מנסה לחפש ניתוח מניה בתוך תיבת המושגים
                if "טסלה" in q or "tsla" in q:
                    match_text = "<b>חברת טסלה (Tesla - TSLA):</b> חלוצת הרכבים החשמליים ומובילה עולמית בטכנולוגיית אוטונומיה ובינה מלאכותית.<br/>" \
                                 "המניה נחשבת לנכס צמיחה אגרסיבי בעל תנודתיות (Beta) גבוהה מאוד, המושפעת מקצב מסירות הרבעון והצהרות מנכ\"ל.<br/>" \
                                 "💡 <b>הכוונה למערכת:</b> תיבה זו מיועדת להסבר מושגים כלליים בשוק. כדי לקבל את <u>הגרף והניתוח הטכני החי והמדויק</u> " \
                                 "של טסלה כרגע (כולל רמות RSI וממוצעים נעים עדכניים), אנא הזן את הסימול <b>TSLA</b> בתיבה הימנית למעלה (<b>'ניתוח מניה בודדת'</b>)."
                elif "אפל" in q or "aapl" in q:
                    match_text = "<b>חברת אפל (Apple - AAPL):</b> ענקית טכנולוגיה המהווה עוגן מרכזי במדדי המניות המובילים בארה\"ב (S&P500 ו-NASDAQ).<br/>" \
                                 "המניה נחשבת לחוף מבטחים פיננסי בזכות תזרים מזומנים עצום, רכישה עצמית קבועה של מניות (Buyback) ונאמנות לקוחות גבוהה.<br/>" \
                                 "💡 <b>הכוונה למערכת:</b> תיבה זו מיועדת להסבר מושגים כלליים בשוק. כדי לקבל את <u>הגרף והניתוח הטכני החי והמדויק</u> " \
                                 "של אפל כרגע (כולל רמות RSI וממוצעים נעים עדכניים), אנא הזן את הסימול <b>AAPL</b> בתיבה הימנית למעלה (<b>'ניתוח מניה בודדת'</b>)."
                elif "אנבידיה" in q or "nvda" in q:
                    match_text = "<b>חברת אנבידיה (Nvidia - NVDA):</b> המובילה הבלתי מעורערת של מהפכת השבבים והמעבדים הגרפיים המניעים את עולמות ה-AI.<br/>" \
                                 "המניה מציגה צמיחה פנומנלית בהכנסות אך מתאפיינת במכפילים גבוהים, דבר שמייצר תנודתיות קיצונית סביב פרסום הדוחות.<br/>" \
                                 "💡 <b>הכוונה למערכת:</b> תיבה זו מיועדת להסבר מושגים כלליים בשוק. כדי לקבל את <u>הגרף והניתוח הטכני החי והמדויק</u> " \
                                 "של אנבידיה כרגע (כולל רמות RSI וממוצעים נעים עדכניים), אנא הזן את הסימול <b>NVDA</b> בתיבה הימנית למעלה (<b>'ניתוח מניה בודדת'</b>)."
                elif "מיקרוסופט" in q or "msft" in q:
                    match_text = "<b>חברת מיקרוסופט (Microsoft - MSFT):</b> ענקית תוכנה, ענן ארגוני (Azure) ושותפה אסטרטגית מרכזית בפיתוח מודלי OpenAI.<br/>" \
                                 "מציגה מודל יציב של הכנסות חוזרות (SaaS) ונחשבת לאחת החברות בעלות שווי השוק והחוסן הפיננסי הגבוהים ביותר בעולם.<br/>" \
                                 "💡 <b>הכוונה למערכת:</b> תיבה זו מיועדת להסבר מושגים כלליים בשוק. כדי לקבל את <u>הגרף והניתוח הטכני החי והמדויק</u> " \
                                 "של מיקרוסופט כרגע (כולל רמות RSI וממוצעים נעים עדכניים), אנא הזן את הסימול <b>MSFT</b> בתיבה הימנית למעלה (<b>'ניתוח מניה בודדת'</b>)."
                
                # ב. סריקת מונחים פיננסיים כלליים במקרה שזו לא מניה ספציפית
                else:
                    FINANCIAL_KB = {
                        "rsi": (
                            "<b>מדד העוצמה היחסי (RSI):</b> מתנד טכני המודד את מהירות ועוצמת שינויי המחיר של הנכס בסולם שבין 0 ל-100.<br/>" \
                            "• ערך מעל 70 מסמן מצב של 'קניית יתר' (Overbought), המעיד על מתיחות הגרף וסיכון פוטנציאלי לתיקון טכני קרוב מטה.<br/>" \
                        ...
                        "לונג": (
                            ...
                            "• פרופיל הסיכון מוגבל לחלוטין: ההפסד המקסימלי מוגבל לסכום ההשקעה ההתחלתי בלבד במקרה הקיצוני שבו החברה מגיעה לאפס.<br/>" \
                            "בניתוח טכני, כניסה ללונג מתבצעת לרוב במגמה עולה מובהקת, מעל ממוצעים נעים מרכזיים או לאחר סיום תיקון באזורי תמיכה."
                        ),
                        "אופציות": (
                            "<b>אופציות (Options):</b> מכשירים פיננסיים נגזרים המעניקים למחזיק זכות (ללא חובה) לקנות או למכור נכס בסיס במחיר קבוע מראש.<br/>" \
                            "• אופציית Call: מעניקה זכות לקניית המניה ומיועדת לסוחרים המזהים פוטנציאל לעליית שערים חדה בתוך חלון זמן מוגדר.<br/>" \
                            "• אופציית Put: מעניקה זכות למכירת המניה ומיועדת לסוחרים הצופים ירידות שערים או מעוניינים לבטח תיק השקעות קיים.<br/>" \
                            "יחס ה-Put/Call משמש כלי סנטימנט קריטי בשוק: ערכים גבוהים מ-1 מעידים על פחד, וערכים נמוכים מעידים על אופטימיות יתר."
                        ),
                        "תמיכה": (
                ...
                # ג. תשובת גיבוי מעמיקה ודינמית לכל שאלה כללית אחרת בנושאי שוק ההון והבורסה
                if not match_text:
