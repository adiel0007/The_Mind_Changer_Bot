...
# ── עיצוב גלובלי לרכיבי Streamlit המובנים (התאמה לעיצוב השחור/זהב שלך) ──
st.markdown("""
<style>
...
/* עיצוב הטאבים המובנים של Streamlit */
...
/* עיצוב כפתורים מובנים לפי קבוצות */
div.stButton > button {
    width: 100% !important;
...
/* התאמת צבעים לכפתורים ספציפיים */
.long-btn-container div[data-testid="stButton"] button { background-color: #16a34a !important; color: white !important; }
.short-btn-container div[data-testid="stButton"] button { background-color: #dc2626 !important; color: white !important; }
.gold-btn-container div[data-testid="stButton"] button { background-color: #c9a84c !important; color: #0a0a08 !important; }
</style>
""", unsafe_allow_html=True)

# ── ה-CSS המקורי שלך לשימוש חוזר בתוך הרכיבים ──
SHARED_CSS = """
...
body{background:transparent;color:var(--text);font-family:'Inter',sans-serif;direction:rtl;margin:0}
...
#hero{display:grid;grid-template-columns:1fr 1fr;align-items:center;padding:60px 40px 48px;gap:40px;position:relative;overflow:hidden;}
...
# ── 2. רכיב הרדאר המרכזי (מנוהל נייטיב בתוך אותו עמוד ב-Streamlit) ──
st.markdown('<div style="padding: 20px 40px 10px 40px; max-width: 1200px; margin: 0 auto;">', unsafe_allow_html=True)
...
with tab_long:
    ...
    with col1:
        st.markdown(f"""<style>{SHARED_CSS}</style>
        ...
        st.markdown('<div class="long-btn-container">', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <html lang="he" dir="rtl"><head><style>{SHARED_CSS}</style></head><body>
        <div class="results-panel" style="margin-top:15px;">
          <div class="results-header"><div class="results-title">תוצאות סריקה</div><div class="results-count">{long_count}</div></div>
          {long_cards}
        </div></body></html>"""
        st.components.v1.html(res_html, height=360, scrolling=True)
...
with tab_short:
    ...
    with col1:
        ...
        st.markdown('<div class="short-btn-container">', unsafe_allow_html=True)
    ...
    with col2:
        ...
        res_html_short = f"""<!DOCTYPE html>
        <html lang="he" dir="rtl"><head><style>{SHARED_CSS}</style></head><body>
        <div class="results-panel" style="margin-top:15px;">
          <div class="results-header"><div class="results-title">תוצאות סריקה</div><div class="results-count">{short_count}</div></div>
          {short_cards}
        </div></body></html>"""
        st.components.v1.html(res_html_short, height=360, scrolling=True)
...
with tab_ai:
    ...
    with col1:
        ...
        if st.session_state.analysis:
            ...
            st.components.v1.html(f"<!DOCTYPE html><html lang='he' dir='rtl'><head><style>{SHARED_CSS}</style></head><body>{analysis_html}</body></html>", height=380, scrolling=True)
    ...
    with col2:
        ...
        if st.session_state.ai_answer:
            ans_box = f"""<!DOCTYPE html><html lang='he' dir='rtl'><head><style>{SHARED_CSS}</style></head><body>
            ...
            st.components.v1.html(ans_box, height=140, scrolling=False)
