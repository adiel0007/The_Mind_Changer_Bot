...
# ═══════════════════════════════════════════════════════
#  בניית ה-HTML השלם (ללא f-string למניעת שגיאות סינטקס)
# ═══════════════════════════════════════════════════════
...
<nav>
  ...
  <ul class="nav-links">
    <li><a id="n-hero" onclick="goto('hero')" class="active">בית</a></li>
    <li><a id="n-radar" onclick="goto('radar')">רדאר</a></li>
    <li><a id="n-features" onclick="goto('features')">יתרונות</a></li>
    <li><a id="n-how" onclick="goto('how')">תהליך</a></li>
  </ul>
...
<section id="radar">
  <div class="section-wrap">
    ...
    <div class="tab-bar">
      ...
      <button class="tab-btn [AI_ACTIVE]"           id="tbtn-ai"    onclick="switchTab('ai')">🤖 ניתוח AI</button>
    </div>
    ...
    <div class="tab-panel [LONG_ACTIVE]" id="tab-long">
      <div class="radar-layout">
        ...
        <div class="results-panel">
          ...
          <div id="res-long">[LONG_CARDS]</div>
        </div>
      </div>
    </div>
    ...
    <div class="tab-panel [SHORT_ACTIVE]" id="tab-short">
      <div class="radar-layout">
        ...
        <div class="results-panel">
          ...
          <div id="res-short">[SHORT_CARDS]</div>
        </div>
      </div>
    </div>
    ...
    <div class="tab-panel [AI_ACTIVE]" id="tab-ai">
      <div class="ai-grid">
        <div class="panel-card">
          ...
          <div id="res-analyze">[ANALYSIS_CONTENT]</div>
        </div>
        <div class="panel-card">
...
<script>
const QUOTES  = [QUOTES_PLACEHOLDER];
const INDICES = [INDICES_PLACEHOLDER];
const STOCKS  = [STOCKS_PLACEHOLDER];
...
function goto(id){
  ...
  const navs = ['hero', 'radar', 'features', 'how'];
  navs.forEach(n => {
...
function triggerScan(mode){
  ...
  for(const btn of buttons){
    if(btn.innerText.trim()===label){ btn.click(); found = true; break; }
  }
...
function answerQ(){
  ...
  const m=QA.find(function(item){return q.includes(item[0]);});
...
document.getElementById('ticker-input').addEventListener('keydown',function(e){if(e.key==='Enter')triggerAnalyze();});
document.getElementById('ai-q').addEventListener('keydown',function(e){if(e.key==='Enter')answerQ();});

buildTape(); buildHero();
switchTab('[ACTIVE_TAB_PLACEHOLDER]');
</script>
</body>
</html>"""

# ═══════════════════════════════════════════════════════
#  הזרקת הנתונים בצורה בטוחה ללא שימוש ב-f-string
# ═══════════════════════════════════════════════════════
long_results_count = f"{len(st.session_state.long_results)} מניות" if st.session_state.long_results is not None else "—"
short_results_count = f"{len(st.session_state.short_results)} מניות" if st.session_state.short_results is not None else "—"

html = html_template.replace("[QUOTES_PLACEHOLDER]", quotes_json[1:-1])
html = html.replace("[INDICES_PLACEHOLDER]", indices_json[1:-1])
html = html.replace("[STOCKS_PLACEHOLDER]", stocks_json[1:-1])
html = html.replace("[ACTIVE_TAB_PLACEHOLDER]", active_tab)
html = html.replace("[LONG_ACTIVE]", "active" if active_tab == "long" else "")
html = html.replace("[SHORT_ACTIVE]", "active" if active_tab == "short" else "")
html = html.replace("[AI_ACTIVE]", "active" if active_tab == "ai" else "")
html = html.replace("[LONG_COUNT]", long_results_count)
html = html.replace("[SHORT_COUNT]", short_results_count)
html = html.replace("[LONG_CARDS]", render_cards(st.session_state.long_results, 'long'))
html = html.replace("[SHORT_CARDS]", render_cards(st.session_state.short_results, 'short'))
html = html.replace("[ANALYSIS_CONTENT]", render_analysis(st.session_state.analysis))
