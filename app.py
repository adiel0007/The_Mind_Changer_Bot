...
def do_scan(mode):
    ...
    for i, ticker in enumerate(tickers):
        status.markdown(
            f"<span style='color:#c9a84c;font-size:0.82rem;'>🔍 סורק {ticker}... ({i+1}/{total})</span>",
            unsafe_allow_html=True)
...
if "analyze" in st.query_params:
    ticker = st.query_params["analyze"].upper().strip()
    st.query_params.clear()
...
ticker_val  = st.session_state.ticker_hidden or ""
...
function buildTape() {{
  ...
  document.getElementById('tape').innerHTML = full.map(t =>
    ...
    t.price + ' ' + (t.change_pct >= 0 ? '+' : '') + t.change_pct + '%</span>' +
    '</div>'
  ).join('');
}}
...
function answerQ() {{
  ...
  document.getElementById('res-ai').innerHTML =
    ...
    '<div class="ai-response-text">' + ans + '</div>' +
    '</div>';
}}
