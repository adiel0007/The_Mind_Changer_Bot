        ...
        with open(os.devnull, 'w') as devnull:
            with contextlib.redirect_stderr(devnull), contextlib.redirect_stdout(devnull):
                try:
                    ...
                    # אנו מושכים חודשיים של דאטה כדי שיהיה לנו מידע מלא על פתיחה/סגירה של הימים האחרונים
                    chunk_data = yf.download(tickers_str, period="2mo", interval="1d", group_by='ticker', auto_adjust=False, progress=False, ignore_tz=True)
                except:
            ...
            try:
                if not chunk_data.empty and (ticker in chunk_data.columns.levels[0] if isinstance(chunk_data.columns, pd.MultiIndex) else ticker in chunk_data.columns):
                    ...
                    if not df_ticker.empty and len(df_ticker) >= 14:
                        ...
                        # 📉 תנאי רדאר שורט סווינג המעודכן שלך
                        elif last_price < ma9 and volume > 1000000:
                            # 1. פילטר חדש: מציג רק מניות עם RSI מעל 30
                            if rsi > 30:
                                # 2. פילטר חדש: בדיקה ששני הנרות היומיים האחרונים נסגרו שליליים (Close < Open)
...
with tab1:
    # 🛠️ תיקון 1: הזזת הסמיילי 📉 לצד שמאל של המשפט (ב-RTL)
...
if run_radar:
    # 🛠️ תיקון 3 & 2: איפוס המצב לפני סריקה חדשה (לפתור בעיית תקיעה ומחיקת לוגים)
    st.session_state.short_list = []
    st.session_state.long_list = []
    st.session_state.radar_scanned = False
    ...
    def update_progress(current, total, ticker):
        ...
        # 🛠️ תיקון 3: הבטחה ששורת הטעינה הלבנה היא זו שמופיעה
    ...
    # 🛠️ תיקון 2: מחיקת הבלוק של שלבי הריצה שמופיע מלמטה
    # (כל הבלוק שהיה קשור ל-steps_log ו-t_step נמחק מכאן)
    
    st.session_state.short_list = temp_short
    st.session_state.long_list = temp_long
    st.session_state.radar_scanned = True
    
    # ריענון הממשק להצגת התוצאות בטאבים
    st.rerun()

# 🛠️ תיקון 2: מחיקת הבלוק שמופיע מלמטה ומציג את יומן השלבים
