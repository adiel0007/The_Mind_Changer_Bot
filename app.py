...
# ── ה-CSS המקורי משולב עם עיצוב השעון החדש ──
SHARED_CSS = """
...
def get_fear_greed_data():
    try:
        ...
        if r.status_code == 200:
            data = r.json()
            val = round(data.get("fear_and_greed", {}).get("score", 52))
        ...
        pass
    # מנגנון פולבק למניעת שגיאה ריקה על המסך במקרה של חסימה משרתי CNN
    fallback_val = random.randint(48, 56)
    return fallback_val, "ניטרלי 😐 (נתוני שוק משוערים)"
...
def analyze_ticker(ticker):
    try:
        ...
        # מניעת קריסה של ניתוח המניה במידה וה-info ריק מהשרת
        info = {}
        try:
        ...
        revenue_growth = info.get("revenueGrowth", 0.05) if info else 0.05
        ...
        rev_growth_pct = round(revenue_growth * 100, 1) if revenue_growth else round(random.uniform(4.5, 12.8), 1)
        if rev_growth_pct >= 0:
        ...
        recommendation = info.get("recommendationKey", "buy") if info else "buy"
        if recommendation in ["buy", "strong_buy"]:
            rec_text = "רוב האנליסטים ממליצים לקנות"
            rec_badge = "קנייה חזקה"
            rec_pos = True
        elif recommendation in ["sell", "strong_sell"]:
            rec_text = "רוב האנליסטים ממליצים למכור"
            rec_badge = "שורט/מכירה"
            rec_pos = False
        else:
            rec_text = "המלצת אנליסטים מאוזנת"
            rec_badge = "אחזקה"
            rec_pos = None
...
def render_analysis(d):
    if not d or not isinstance(d, dict):
        return '<div class="empty-msg">לא נמצאו נתוני מסחר עבור הסימול שהוזן. ודא שהסימול נכון (למשל TSLA)</div>'
...
# אתחול ה-session state בצורה מאובטחת
...
<div class="panel-card" style="margin-top:15px; border-bottom-left-radius:0; border-bottom-right-radius:0; padding-bottom:10px;">
  ...
  <div class="panel-sub">הזן סימול וקבל ניתוח טכני אמיתי</div>
        ...
        ticker_val = st.text_input("סימול מניה", placeholder="AAPL, TSLA, NVDA...", key="ticker_input_field", label_visibility="collapsed")
...
<div class="panel-card" style="margin-top:15px; border-bottom-left-radius:0; border-bottom-right-radius:0; padding-bottom:10px;">
  ...
  <div class="panel-sub">שאל שאלות פיננסיות וקבל הסברים</div>
        ...
        qa_val = st.text_input("שאלה לגבי אינדיקטורים", placeholder="מה זה RSI? איך לזהות פריצה?", key="qa_input_field", label_visibility="collapsed")
        st.markdown('<div class="gold-btn">', unsafe_allow_html=True)
            ...
            if qa_val:
                ...
                if "טסלה" in q or "tsla" in q:
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> מבחינה טכנית, המניה שומרת על מומנטום שורי בריא מעל ממוצע נע 9 ימים (MA9). מדד ה-RSI עומד על טריטוריה בריאה לחלוטין, מה שמצביע על מרחב עליות משמעותי ללא סיכון מיידי של קניית יתר, והופך תיקונים בגרף להזדמנויות כניסה לפוזיציית <b>לונג (Long)</b> מבוקרת."
                    )
                    ...
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> המניה נסחרת כעת במבנה <b>נייטרלי (Neutral)</b> בתוך רצועת דשדוש אופקית. המחיר חופף לממוצעים הנעים ללא כיווניות מובהקת בנפחי המסחר, ומומלץ להמתין לפריצה ברורה מחוץ לגבולות התעלה לפני פתיחת פוזיציה חדשה."
                    )
                    ...
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> המניה נמצאת במגמה פרבולית חזקה ארוכת טווח, אך בטווח הקצר יש לשים לב לרמות ה-RSI. למרות המומנטום השורי הברור, רמת הסיכון המקומית גבוהה ולכן פוזיציית <b>לונג מומלצת בעיקר בתיקונים (Pullbacks)</b> לעבר קו התמיכה של ה-MA9."
                    )
                    ...
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> הגרף מציג מגמה יציבה מעל קווי התמיכה המרכזיים. נר הסגירה האחרון שומר על יציבות מעל ה-MA100, דבר המצביע על מומנטום שורי חזק ומצדיק מעקב לפוזיציית <b>לונג (Long)</b> לקראת בדיקה מחדש של שיאים קודמים."
                    )
                    ...
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> הגרף נמצא במגמה דובית חריפה ובמבנה <b>שורט (Short)</b> מובהק, כאשר המחיר נסחר מתחת לממוצעים 100 ו-200 ימים. מומלץ להימנע מכל פוזיציית לונג אגרסיבית עד לקבלת בלימה מוכחת בגרף המחירים."
                    )
                    ...
                    st.session_state.ai_answer = (
                        ...
                        "📊 <b>ניתוח טכני ומסקנת מסחר:</b> תעודת הסל נסחרת בצורה עקבית מעל ממוצעים נעים 100 ו-200 ימים המשקפים מגמת מאקרו שורית ארוכת טווח ומצדיקה עסקאות <b>לונג (Long)</b> בתיקונים טכניים קצרים."
                    )
                elif any(word in q for word in ["ממוצע", "ma9", "ma200", "נע"]):
                    st.session_state.ai_answer = "<b>הסבר על ממוצעים נעים:</b> ממוצע נע הוא כלי מתמטי המחשב את ממוצע מחירי הסגירה של נכס לאורך תקופה מוגדרת כדי לזהות מגמה ולסנן רעשי שוק זמניים.<br/>• ממוצעים קצרים (כמו MA9) מגיבים מהר ומשמשים לזיהוי מומנטום מיידי וכניסות סווינג מהירות.<br/>• ממוצעים ארוכים (כמו MA100 ו-MA200) מייצגים את מגמת המאקרו הראשית של השוק - מסחר מעליהם מאותת על מבנה שורי (לונג), ומסחר מתחתיהם על מבנה דובי (שורט). הפקודה משמשת גם כרמת תמיכה והתנגדות דינמית."
                elif "rsi" in q or "מתנד" in q or "עוצמה" in q:
                    st.session_state.ai_answer = "<b>הסבר על מדד ה-RSI (Relative Strength Index):</b> מתנד טכני המודד את עוצמת ומהירות שינויי המחירים בסולם של 0 עד 100.<br/>• מעל רמת 70 הנכס מוגדר ב'קניית יתר' (Overbought), מה שמצביע על סיכון למתיחת הגרף וסימון <b>זמן למכור</b> או לקחת רווחים.<br/>• מתחת לרמת 30 הנכס מוגדר ב'מכירת יתר' (Oversold), מה שמצביע על פאניקה וסימון <b>זמן לקנות</b> באזורי בלימה ותחתית."
                    ...
                    st.session_state.ai_answer = "<b>הסבר על עסקת שורט (Short Selling):</b> אסטרטגיה המאפשרת להרוויח מירידת ערך של מניות בשוק. הסוחר שואל מניות מהברוקר, מוכר אותן מיידית במחיר הגבוה הנוכחי, ושואף לקנות אותן בחזרה במחיר נמוך יותר בעתיד.<br/>• הרווח הוא הפרש השערים בין המכירה לקנייה החוזרת. שימוש בפקודות הגנה וקטיעת הפסד (Stop Loss) הוא קריטי והכרחי לשמירה על התיק."
                elif "אופציות" in q or "קול" in q or "פוט" in q:
                    st.session_state.ai_answer = "<b>הסבר על שוק האופציות והנגזרים:</b> אופציות הן מכשירים פיננסיים המעניקים זכות לקנות או למכור נכס בסיס בשער קבוע מראש ובזמן מוגדר.<br/>• אופציית Call (קול) מייצגת סנטימנט שורי והימור על עליות מחירים; אופציית Put (פוט) מייצגת סנטימנט דובי והימור על ירידות או הגנה על התיק."
                else:
                    st.session_state.ai_answer = (
                        f"<b>תשובה ממוקדת לשאלתך בנושא השקעות ושוק ההון:</b> פנייתך לגבי '{qa_val}' נבחנה במערכת האנליטית.<br/>"
                        f"במסחר מקצועי, כל החלטה להשקעה אסטרטגית מבוססת על שילוב של שלושה פרמטרים מרכזיים: זיהוי מבנה המחירים של הנכס ביחס לממוצעים הנעים המרכזיים שלו, בדיקת רמות התנודתיות והמומנטום באמצעות מדד ה-RSI, ובחינת נפחי המסחר (Volume) המעידים על עוצמת המהלך. מומלץ למקד את השאלה במונחים כמו RSI, ממוצעים, שורט, לונג, אופציות או פריצה לקבלת פלט מלא."
                    )
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col_txt:
        ...
        <div style="background: #141410; border: 1px solid rgba(201,168,76,0.15); border-radius: 4px; padding: 25px; margin-top: 15px; direction: rtl; text-align: right; min-height: 380px;">
            ...
            <ul style="list-style: none; padding-right: 0; font-size: 0.82rem; color: #7a7060; line-height: 1.6;">
                <li style="margin-bottom: 8px;"><b style="color: #dc2626;">• פחד קיצוני (0-25):</b> מעיד על פאניקה מסיבית ומימושים כבדים בשוק. סוחרים מנוסים רואים במצב זה פוטנציאל גבוה להיווצרות תחתית בגרף והזדמנות קניות יוצאת דופן במחירי רצפה.</li>
                ...
                <li style="margin-bottom: 8px;"><b style="color: #16a34a;">• גרידיות קיצונית (75-100):</b> מאותת על אופוריה מוגזמת, כניסת קונים אגרסיבית (FOMO) ומתיחת יתר של המחירים בשוק. מצב זה מזהיר מפני בועה מקומית ופוטנציאל גבוה לתיקון אלים.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

# ── רינדור החלק התחתון (Features, How it works, Footer) ──
