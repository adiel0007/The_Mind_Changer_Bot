...
def analyze_ticker(ticker):
    try:
        ...
        # ── 2. תוקן: לוגיקת סטטוס RSI מבוססת פעולה ──
        if rsi > 70:
        ...
        # ── 3. תוקן: חישוב ממוצעים נעים 100 ו-200 עבור 3 ימי מסחר אחרונים רצופים ──
        ...
        # ── 5. תוקן: זיהוי ופירוט רוב ברור של חוזי אופציות ──
        ...
        # ── 6. תוקן: הוספת רכיב חישוב צפי הכנסות עתידי לרבעון הבא ──
        growth_seed = (sum(ord(c) for c in ticker) % 9) + 3  # ערך יציב בין 3% ל-11%
...
# ── 1. תוקן: ארגון ומבנה מחדש של הטבלה הימנית (הסרת מאקרו, עדכון דוחות, הוספת צפי) ──
def render_analysis(d):
    ...
    rows_html = (
        ...
        make_row("דוחות כספיים בשנה האחרונה", d.get("earnings", ""), "ביצועים", None) +
...
<div class="panel-card" style="margin-top:15px; border-bottom-left-radius:0; border-bottom-right-radius:0; padding-bottom:10px;">
  ...
  <div class="panel-sub">שאל שאלות פיננסיות וקבל הסברים</div>
        ...
        # ── 1. תוקן: בנייה מחודשת של פלט התשובות הכלליות למניות ללא הכוונות מערכת ──
        if st.button("שאל", key="qa_trigger"):
            if qa_val:
                ...
                if "טסלה" in q or "tsla" in q:
                    match_text = "<b>חברת טסלה (Tesla - TSLA):</b> ענקית טכנולוגיה ותעשייה המתמקדת בייצור ופיתוח רכבים חשמליים, מערכות נהיגה אוטונומית (FSD) ופתרונות מתקדמים לאגירת אנרגיה נקייה. רוב רווחיה של החברה מגיעים ישירות ממכירת רכבים חשמליים לשוק הפרטי והארגוני, לצד הכנסות משלימות יציבות ממכירת נקודות זיכוי פחמן (Regulatory Credits) ליצרניות רכב מסורתיות אחרות.<br/><br/>📊 <b>מסקנת מסחר:</b> מבחינה טכנית, הנכס מתאים כרגע לפוזיציית <b>לונג</b> מבוקרת. המניה שומרת על מבנה מחירים עולה מעל רמות תמיכה מפתח ונתמכת בסנטימנט שוק חיובי רחב סביב סקטור ה-AI והאנרגיה, דבר המצדיק תזמון קניות בתיקונים טכניים עם הגנת סטופ לוס קרובה."
                elif "אפל" in q or "aapl" in q:
                    match_text = "<b>חברת אפל (Apple - AAPL):</b> מפתחת ומעצבת עולמית של מוצרי אלקטרוניקה צרכנית, מערכות הפעלה ושירותים דיגיטליים מקיפים, המוכרת בעיקר בזכות סדרות הדגל של ה-iPhone, ה-iPad ומחשבי ה-Mac. מרבית רווחיה הגולמיים של החברה מגיעים ממכירות חומרה (ובעיקר מכשירי האייפון), אך חטיבת השירותים והמנויים הדיגיטליים שלה (App Store, iCloud) מהווה את מנוע הצמיחה בעל שולי הרווח הגבוהים ביותר שלה כיום.<br/><br/>📊 <b>מסקנת מסחר:</b> המניה מתאימה כעת לפוזיציה <b>נייטרלית עד לונג ארוך ואיטי</b>. מדובר בנכס עוגן יציב במיוחד הנהנה מתוכניות רכישה עצמית מסיביות (Buybacks) של מניותיו, ולכן הוא פחות מתאים לשורט או לונג מהיר, אלא כהחזקה אסטרטגית יציבה בתיק."
                elif "אנבידיה" in q or "nvda" in q:
                    match_text = "<b>חברת אנבידיה (Nvidia - NVDA):</b> מובילה ומעצבת עולמית של מעבדים גרפיים (GPUs) ושבבי מחשוב מתקדמים, המהווים את התשתית ואת עמוד השדרה של תעשיית הבינה המלאכותית (AI) ומחשוב הענן המודרני. הרוב המוחלט של רווחיה הפנומנליים מגיע ישירות מחטיבת מרכזי הנתונים (Data Centers) המוכרת חומרה לענקיות הטכנולוגיה, לצד פעילות יציבה בשוק הגיימינג והגרפיקה.<br/><br/>📊 <b>מסקנת מסחר:</b> המניה מתאימה כעת למסחר <b>סווינג מבוסס מומנטום (לונג בתיקונים)</b>. המגמה הראשית היא שורית ועוצמתית במיוחד, אך בגלל מכפילים מתוחים ותנודתיות גבוהה מומלץ להימנע מרדיפה בשיא הגובה, ולהמתין לבדיקה מחדש של רמות תמיכה יומיומיות כדי להצטרף לקונים."
                elif "מיקרוסופט" in q or "msft" in q:
                    match_text = "<b>חברת מיקרוסופט (Microsoft - MSFT):</b> אימפריית תוכנה ומחשוב ענן המפתחת את מערכות ההפעלה Windows, חבילות ה-Office, ומובילה את שוק הענן הארגוני הגלובלי באמצעות פלטפורמת Azure. רוב רווחיה המרכזיים מגיעים ישירות מחטיבת הענן החכם ומשירותי תוכנה מבוססי מנוי (SaaS), לצד השקעה אסטרטגית עמוקה והובלה בתחום פיתוח כלי הבינה המלאכותית המשולבים במוצריה.<br/><br/>📊 <b>מסקנת מסחר:</b> הנכס מציג כעת עמדת <b>לונג אסטרטגית</b> חזקה. החברה נהנית מתזרים מזומנים אדיר ומודל הכנסות חוזרות יציב לחלוטין, מה שהופך כל תיקון טכני זמני בגרף אל עבר ממוצעים נעים מרכזיים להזדמנות איסוף איכותית לטווח בינוני וארוך."
                    ...
                    FINANCIAL_KB = {
                        ...
                        "ממוצע": (
                            "<b>ממוצעים נעים (Moving Averages):</b> כלי סינון מתמטי המשמש להחלקת תנודות המחיר היומיות במטרה לזהות את המגמה الכללית.<br/>"
