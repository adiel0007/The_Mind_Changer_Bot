...
def download_market_data_safely(ticker_list, status_container, progress_bar):
    temp_short = []
    temp_long = []
    chunk_size = 10
...
with tab1:
    ...
    if st.session_state.radar_scanned and st.session_state.short_list:
        # 🛠️ תיקון אבסולוטי לבאג שרואים ב-image_410c8a.jpg: שורה שטוחה ונקייה ללא רווחים מוקדמים למניעת קריסת מארק-דאון
...
if run_radar:
    ...
    temp_short, temp_long = download_market_data_safely(tickers, status_text, progress_bar)
